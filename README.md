<h1 align="center">NYCU Coursework Overview</h1>

## Core Courses

### Introduction to Operating System
- [Homeworks](https://github.com/heng907/Introduction-to-Operating-Systems.git)
- Topics: Process scheduling, memory management, system calls, file systems

### Computer Organization
- [Homeworks](https://github.com/heng907/Computer-Oraganization.git)
- Topics: ISA design, datapath & control unit, pipelining

### Data Structures and OOP
- [Homeworks](https://github.com/heng907/Data-Structures-and-Object-oriented-Programming.git)
- Topics: Linked lists, trees, OOP design, class hierarchies

### Introduction to Algorithm
- [Homeworks](https://github.com/heng907/Introduction-to-Algorithm.git)
- Topics: Divide and conquer, dynamic programming, graph algorithms

### Probability
- Topics: Random variables, distributions, Bayesian inference

### Discrete Mathematics & Linear Algebra
- Topics: Multivariable calculus, vector spaces, eigenvalues, matrix operations
  

## AI & Vision

### Introduction to Artificial Intelligence
- [Project](https://github.com/heng907/2024-AI-Final-Project.git)
- [Homeworks](https://github.com/heng907/Introduction-to-AI.git)
- Topics: Search, knowledge representation, logic, LLM-based applications

### Introduction to Image Processing
- Homeworks: [Traditional Image-Processing Algorithms Practice](https://github.com/heng907/Introduction-to-Image-Processing.git)
- Topics: Filtering, Laplacian pyramids, contrast enhancement, auto white balancing


## Networks & Systems

### Introduction to Computer Networks
- [Homeworks](https://github.com/heng907/Introduction-to-Computer-Network.git)
- Topics: OSI model, TCP/IP stack, routing, congestion control

### System Administration (SA)
- Project: FreeBSD/Linux Labs
- Topics: User management, service configuration, scripting, cron jobs

### Introduction to Network Programming
- [Homeworks](https://github.com/heng907/Network-Programming.git)
- Topics: TCP/UDP sockets, concurrency, client-server architecture


## Security & Crypto
### Networks Security
- [Homeworks](https://github.com/heng907/Network-Security.git)
- Topics: Cryptography, various kinds of Security

### Cryptography Engineering
- [Homeworks](https://github.com/heng907/Cryptography-Engineering.git)
- Topics: Practical implementations of cryptographic algorithms, including AES (ECB/CBC), RSA encryption & signature schemes,  etc.



## Data & Visualization

### Introduction to Database Systems
- [Project](https://github.com/2023FallDatabase/FinalProject.git)
- [Homeworks](https://github.com/heng907/Introduciotn-to-Database.git)
- Topics: ER modeling, SQL joins, indexing, transactions

### Numerical Methods
- [Homeworks](https://github.com/heng907/Numerical-Methods.git)
- Topics: Iterative solvers, interpolation, numerical integration
